...
status_t MCP3421_SetGainSampleRate(PGAGain_t gain, Samplerate_t samplerate) {
  ...
  return I2CTransfer(GetI2C2_Handle(), MCP3421_addr, kLPI2C_Write, 0, 0, &MCP3421_cfg, 1);
}

status_t MCP3421_StartOneShot() {
  uint8_t out = MCP3421_cfg | (1 << ReadyBit_offset); //set status bit to start one shot conversion
  return I2CTransfer(GetI2C2_Handle(), MCP3421_addr, kLPI2C_Write, 0, 0, &out, 1);
}

status_t MCP3421_ReadADC(int32_t *value) {
  uint8_t in[3];
  status_t status = I2CTransfer(GetI2C2_Handle(), MCP3421_addr, kLPI2C_Read, 0, 0, in, 3);
