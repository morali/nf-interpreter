...
void tsm_set_timeout_handler(tsm_timeout_function pFunction) { Timeout_Function = pFunction; }
...
void tsm_set_confirmed_unsegmented_transaction(uint8_t invokeID, BACNET_ADDRESS *dest, BACNET_NPDU_DATA *ndpu_data, uint8_t *apdu, uint16_t apdu_len) {
  uint16_t j = 0;
  uint8_t index;
...
bool tsm_get_transaction_pdu(uint8_t invokeID, BACNET_ADDRESS *dest, BACNET_NPDU_DATA *ndpu_data, uint8_t *apdu, uint16_t *apdu_len) {
  uint16_t j = 0;
  uint8_t index;
  bool found = false;
...
void tsm_init() {
  TSM_List = (BACNET_TSM_DATA *)malloc(MAX_TSM_TRANSACTIONS * sizeof(BACNET_TSM_DATA));
  memset(TSM_List, 0, MAX_TSM_TRANSACTIONS * sizeof(BACNET_TSM_DATA));
}
...
int datalink_send_pdu(BACNET_ADDRESS *dest, BACNET_NPDU_DATA *npdu_data, uint8_t *pdu, unsigned pdu_len) {
  (void)dest;
  (void)npdu_data;
  (void)pdu;
  (void)pdu_len;
...
/* dummy function stubs */
void datalink_get_broadcast_address(BACNET_ADDRESS *dest) { (void)dest; }
