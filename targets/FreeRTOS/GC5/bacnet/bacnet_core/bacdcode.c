...
int decode_tag_number_safe(uint8_t *apdu, uint32_t apdu_len_remaining, uint8_t *tag_number) {
...
bool decode_is_opening_tag(uint8_t *apdu) { return (bool)((apdu[0] & 0x07) == 6); }

/* from clause 20.2.1.3.2 Constructed Data */
/* returns the number of apdu bytes consumed */
bool decode_is_closing_tag(uint8_t *apdu) { return (bool)((apdu[0] & 0x07) == 7); }
...
int decode_tag_number_and_value_safe(uint8_t *apdu, uint32_t apdu_len_remaining, uint8_t *tag_number, uint32_t *value) {
  int len = 0;
...
bool decode_is_context_tag_with_length(uint8_t *apdu, uint8_t tag_number, int *tag_length) {
  uint8_t my_tag_number = 0;
...
int encode_context_boolean(uint8_t *apdu, uint8_t tag_number, bool boolean_value) {
  int len = 0; /* return value */
...
int decode_context_boolean2(uint8_t *apdu, uint8_t tag_number, bool *boolean_value) {
  int len = 0;
...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
int encode_application_null(uint8_t *apdu) { return encode_tag(&apdu[0], BACNET_APPLICATION_TAG_NULL, false, 0); }

int encode_context_null(uint8_t *apdu, uint8_t tag_number) { return encode_tag(&apdu[0], tag_number, true, 0); }
...
/* returns the number of apdu bytes consumed */
int decode_bitstring(uint8_t *apdu, uint32_t len_value, BACNET_BIT_STRING *bit_string) {
  int len = 0;
...
int decode_context_bitstring(uint8_t *apdu, uint8_t tag_number, BACNET_BIT_STRING *bit_string) {
  uint32_t len_value;
  int len = 0;

  if (decode_is_context_tag(&apdu[len], tag_number) && !decode_is_closing_tag(&apdu[len])) {
...
int encode_bitstring(uint8_t *apdu, BACNET_BIT_STRING *bit_string) {
  ...
  if (bitstring_bits_used(bit_string) == 0) {
    ...
    remaining_used_bits = (uint8_t)(bitstring_bits_used(bit_string) - ((used_bytes - 1) * 8));
...
int encode_context_bitstring(uint8_t *apdu, uint8_t tag_number, BACNET_BIT_STRING *bit_string) {
  int len = 0;
  uint32_t bit_string_encoded_length = 1; /* 1 for the bits remaining octet */
...
int decode_object_id_safe(uint8_t *apdu, uint32_t len_value, uint16_t *object_type, uint32_t *instance) {
  if (len_value != 4) {
    return 0;
  } else {
...
int decode_context_object_id(uint8_t *apdu, uint8_t tag_number, uint16_t *object_type, uint32_t *instance) {
  int len = 0;
...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
int encode_application_object_id(uint8_t *apdu, int object_type, uint32_t instance) {
  int len = 0;
...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
int encode_application_octet_string(uint8_t *apdu, BACNET_OCTET_STRING *octet_string) {
  int apdu_len = 0;
...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
int encode_context_octet_string(uint8_t *apdu, uint8_t tag_number, BACNET_OCTET_STRING *octet_string) {
  int apdu_len = 0;

  if (apdu && octet_string) {
    apdu_len = encode_tag(&apdu[0], tag_number, true, octetstring_length(octet_string));
    if ((apdu_len + octetstring_length(octet_string)) < MAX_APDU) {
      apdu_len += encode_octet_string(&apdu[apdu_len], octet_string);
    } else {
      apdu_len = 0;
...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
int decode_octet_string(uint8_t *apdu, uint32_t len_value, BACNET_OCTET_STRING *octet_string) {
  int len = 0; /* return value */
  bool status = false;
...
int decode_context_octet_string(uint8_t *apdu, uint8_t tag_number, BACNET_OCTET_STRING *octet_string) {
  int len = 0; /* return value */
  bool status = false;
  uint32_t len_value = 0;
...
uint32_t encode_bacnet_character_string_safe(uint8_t *apdu, uint32_t max_apdu, uint8_t encoding, char *pString, uint32_t length) {
...
int encode_bacnet_character_string(uint8_t *apdu, BACNET_CHARACTER_STRING *char_string) {
  return (int)encode_bacnet_character_string_safe(apdu, MAX_APDU, characterstring_encoding(char_string), characterstring_value(char_string),
...
int encode_context_character_string(uint8_t *apdu, uint8_t tag_number, BACNET_CHARACTER_STRING *char_string) {
  int len = 0;
  int string_len = 0;
...
int decode_character_string(uint8_t *apdu, uint32_t len_value, BACNET_CHARACTER_STRING *char_string) {
  ...
  status = characterstring_init(char_string, apdu[0], (char *)&apdu[1], len_value - 1);
  if (status) {
    len = (int)len_value;
  }
...
int decode_context_character_string(uint8_t *apdu, uint8_t tag_number, BACNET_CHARACTER_STRING *char_string) {
  ...
  if (decode_is_context_tag(&apdu[len], tag_number) && !decode_is_closing_tag(&apdu[len])) {
    ...
    status = characterstring_init(char_string, apdu[len], (char *)&apdu[len + 1], len_value - 1);
    if (status) {
      len += len_value;
    }
  } else {
...
int encode_application_unsigned(uint8_t *apdu, uint32_t value) {
  ...
  len += encode_tag(&apdu[0], BACNET_APPLICATION_TAG_UNSIGNED_INT, false, (uint32_t)len);
...
int decode_context_enumerated(uint8_t *apdu, uint8_t tag_value, uint32_t *value) {
  ...
  if (decode_is_context_tag(&apdu[len], tag_value) && !decode_is_closing_tag(&apdu[len])) {
    len += decode_tag_number_and_value(&apdu[len], &tag_number, &len_value);
...
/* and 20.2.1 General Rules for Encoding BACnet Tags */
/* returns the number of apdu bytes consumed */
int encode_bacnet_enumerated(uint8_t *apdu, uint32_t value) { return encode_bacnet_unsigned(apdu, value); }
...
int decode_bacnet_time_safe(uint8_t *apdu, uint32_t len_value, BACNET_TIME *btime) {
  if (len_value != 4) {
    btime->hour = 0;
    btime->hundredths = 0;
    btime->min = 0;
...
int decode_context_bacnet_time(uint8_t *apdu, uint8_t tag_number, BACNET_TIME *btime) {
  int len = 0;
...
/* BACnetAddress */
int encode_context_bacnet_address(uint8_t *apdu, uint8_t tag_number, BACNET_ADDRESS *destination) {
  int apdu_len = 0;
...
/* BACnetAddress */
int decode_context_bacnet_address(uint8_t *apdu, uint8_t tag_number, BACNET_ADDRESS *destination) {
  int len = 0;
  int section_length;
...
static void print_apdu(uint8_t *pBlock, uint32_t num) {
  ...
  if (pBlock && num) {
    ...
    for (line = 0; line < lines; line++) {
      ...
      /* hex representation */
      for (i = 0; i < 16; i++) {
        if (((line == (lines - 1)) && (i <= last_line)) || (line != (lines - 1))) {
...
static void testBACDCodeTags(Test *pTest) {
  ...
  for (tag_number = 0;; tag_number++) {
    ...
    for (value = 1;; value = value << 1) {
      ...
      len = decode_tag_number_and_value(&apdu[0], &test_tag_number, &test_value);
      ct_test(pTest, tag_number == test_tag_number);
...
static void testBACDCodeReal(Test *pTest) {
  ...
  ct_test(pTest, memcmp(&real_array, &encoded_array, sizeof(real_array)) == 0);
...
static void testBACDCodeDouble(Test *pTest) {
  ...
  ct_test(pTest, memcmp(&double_array, &encoded_array, sizeof(double_array)) == 0);
...
static void testBACDCodeOctetString(Test *pTest) {
  ...
  diff = memcmp(octetstring_value(&octet_string), &test_value[0], octetstring_length(&octet_string));
  ct_test(pTest, diff == 0);
  ...
  for (i = 0; i < (MAX_APDU - 6); i++) {
    ...
    ct_test(pTest, status == true);
    apdu_len = encode_application_octet_string(&encoded_array[0], &octet_string);
    len = decode_tag_number_and_value(&encoded_array[0], &tag_number, &len_value);
    ct_test(pTest, tag_number == BACNET_APPLICATION_TAG_OCTET_STRING);
    len += decode_octet_string(&encoded_array[len], len_value, &test_octet_string);
    if (apdu_len != len) {
...
static void testBACDCodeCharacterString(Test *pTest) {
  ...
  ct_test(pTest, apdu_len == len);
  diff = memcmp(characterstring_value(&char_string), &test_value[0], characterstring_length(&char_string));
  ct_test(pTest, diff == 0);
  for (i = 0; i < MAX_CHARACTER_STRING_BYTES - 1; i++) {
    ...
    ct_test(pTest, status == true);
    apdu_len = encode_application_character_string(&encoded_array[0], &char_string);
    len = decode_tag_number_and_value(&encoded_array[0], &tag_number, &len_value);
    ct_test(pTest, tag_number == BACNET_APPLICATION_TAG_CHARACTER_STRING);
    len += decode_character_string(&encoded_array[len], len_value, &test_char_string);
    if (apdu_len != len) {
...
static void testBACDCodeObject(Test *pTest) {
  ...
  ct_test(pTest, memcmp(&object_array[0], &encoded_array[0], sizeof(object_array)) == 0);
