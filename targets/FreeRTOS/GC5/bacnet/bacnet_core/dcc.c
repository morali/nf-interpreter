...
BACNET_COMMUNICATION_ENABLE_DISABLE dcc_enable_status(void) { return DCC_Enable_Disable; }

bool dcc_communication_enabled(void) { return (DCC_Enable_Disable == COMMUNICATION_ENABLE); }

/* When network communications are completely disabled,
   only DeviceCommunicationControl and ReinitializeDevice APDUs
   shall be processed and no messages shall be initiated.*/
bool dcc_communication_disabled(void) { return (DCC_Enable_Disable == COMMUNICATION_DISABLE); }
...
bool dcc_communication_initiation_disabled(void) { return (DCC_Enable_Disable == COMMUNICATION_DISABLE_INITIATION); }

/* note: 0 indicates either expired, or infinite duration */
uint32_t dcc_duration_seconds(void) { return DCC_Time_Duration_Seconds; }
...
int dcc_decode_service_request(uint8_t *apdu, unsigned apdu_len, uint16_t *timeDuration, BACNET_COMMUNICATION_ENABLE_DISABLE *enable_disable,
...
int dcc_decode_apdu(uint8_t *apdu, unsigned apdu_len, uint8_t *invoke_id, uint16_t *timeDuration, BACNET_COMMUNICATION_ENABLE_DISABLE *enable_disable,
                    BACNET_CHARACTER_STRING *password) {
  int len = 0;
  unsigned offset = 0;
  ...
  *invoke_id = apdu[2]; /* invoke id - filled in by net layer */
  ...
  if (apdu_len > offset) {
    len = dcc_decode_service_request(&apdu[offset], apdu_len - offset, timeDuration, enable_disable, password);
  }

  return len;
}

void test_DeviceCommunicationControlData(Test *pTest, uint8_t invoke_id, uint16_t timeDuration, BACNET_COMMUNICATION_ENABLE_DISABLE enable_disable,
                                         BACNET_CHARACTER_STRING *password) {
  uint8_t apdu[480] = {0};
  int len = 0;
  int apdu_len = 0;
  ...
  len = dcc_encode_apdu(&apdu[0], invoke_id, timeDuration, enable_disable, password);
  ct_test(pTest, len != 0);
  apdu_len = len;

  len = dcc_decode_apdu(&apdu[0], apdu_len, &test_invoke_id, &test_timeDuration, &test_enable_disable, &test_password);
  ct_test(pTest, len != -1);
...
void test_DeviceCommunicationControl(Test *pTest) {
  ...
  test_DeviceCommunicationControlData(pTest, invoke_id, timeDuration, enable_disable, &password);

  timeDuration = 12345;
  enable_disable = COMMUNICATION_DISABLE;
  test_DeviceCommunicationControlData(pTest, invoke_id, timeDuration, enable_disable, NULL);
