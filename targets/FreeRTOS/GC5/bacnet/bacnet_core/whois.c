...
int whois_encode_apdu(uint8_t *apdu, int32_t low_limit, int32_t high_limit) {
  ...
  if (apdu) {
    ...
    if ((low_limit >= 0) && (low_limit <= BACNET_MAX_INSTANCE) && (high_limit >= 0) && (high_limit <= BACNET_MAX_INSTANCE)) {
...
int whois_decode_service_request(uint8_t *apdu, unsigned apdu_len, int32_t *pLow_limit, int32_t *pHigh_limit) {
  unsigned int len = 0;
...
int whois_decode_apdu(uint8_t *apdu, unsigned apdu_len, int32_t *pLow_limit, int32_t *pHigh_limit) {
  int len = 0;
  ...
  if (apdu_len >= 2) {
    ...
    len = whois_decode_service_request(&apdu[2], apdu_len - 2, pLow_limit, pHigh_limit);
  }
...
void testWhoIs(Test *pTest) {
  ...
  len = whois_decode_apdu(&apdu[0], apdu_len, &test_low_limit, &test_high_limit);
