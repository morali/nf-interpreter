...
/* a simple table for crossing the services supported */
static BACNET_SERVICES_SUPPORTED unconfirmed_service_supported[MAX_BACNET_UNCONFIRMED_SERVICE] = {SERVICE_SUPPORTED_I_AM,
...
void apdu_set_unrecognized_service_handler_handler(confirmed_function pFunction) { Unrecognized_Service_Handler = pFunction; }
    ...
    for (i = 0; i < MAX_BACNET_CONFIRMED_SERVICE; i++) {
      if (confirmed_service_supported[i] == service_supported) {
        ...
        if (Confirmed_Function[i] != NULL) {
          ...
          int len = Routed_Device_Service_Approval(service_supported, 0, NULL, 0);
          if (len > 0)
...
void apdu_set_abort_handler(abort_function pFunction) { Abort_Function = pFunction; }

static reject_function Reject_Function;

void apdu_set_reject_handler(reject_function pFunction) { Reject_Function = pFunction; }
...
void apdu_handler(BACNET_ADDRESS *src, uint8_t *apdu, /* APDU data */
  ...
  if (apdu) {
    ...
    case PDU_TYPE_CONFIRMED_SERVICE_REQUEST:
      (void)apdu_decode_confirmed_service_request(&apdu[0], apdu_len, &service_data, &service_choice, &service_request, &service_request_len);
    ...
    case PDU_TYPE_ERROR:
      ...
      if (service_choice < MAX_BACNET_CONFIRMED_SERVICE) {
        if (Error_Function[service_choice])
          Error_Function[service_choice](src, invoke_id, (BACNET_ERROR_CLASS)error_class, (BACNET_ERROR_CODE)error_code);
      }
      tsm_free_invoke_id(invoke_id);
      break;
