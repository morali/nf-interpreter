...
int reject_decode_service_request(uint8_t *apdu, unsigned apdu_len, uint8_t *invoke_id, uint8_t *reject_reason) {
  int len = 0;
...
int reject_decode_apdu(uint8_t *apdu, unsigned apdu_len, uint8_t *invoke_id, uint8_t *reject_reason) {
  int len = 0;
  ...
  if (apdu_len) {
    if (apdu[0] != PDU_TYPE_REJECT)
      return -1;
    if (apdu_len > 1) {
      len = reject_decode_service_request(&apdu[1], apdu_len - 1, invoke_id, reject_reason);
    }
  }
...
void testReject(Test *pTest) {
  ...
  len = reject_decode_apdu(&apdu[0], apdu_len, &test_invoke_id, &test_reject_reason);
  ct_test(pTest, len != -1);
