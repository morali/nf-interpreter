...
int wp_decode_service_request(uint8_t *apdu, unsigned apdu_len, BACNET_WRITE_PROPERTY_DATA *wpdata) {
  ...
  if (apdu_len && wpdata) {
    ...
    wpdata->application_data_len = bacapp_data_len(&apdu[len], apdu_len - len, (BACNET_PROPERTY_ID)property);
...
void testWritePropertyTag(Test *pTest, BACNET_APPLICATION_DATA_VALUE *value) {
  ...
  wpdata.application_data_len = bacapp_encode_application_data(&wpdata.application_data[0], value);
  ...
  len = bacapp_decode_application_data(test_data.application_data, test_data.application_data_len, &test_value);
  ...
  case BACNET_APPLICATION_TAG_UNSIGNED_INT:
    ct_test(pTest, test_value.type.Unsigned_Int == value->type.Unsigned_Int);
    break;
  case BACNET_APPLICATION_TAG_SIGNED_INT:
    ct_test(pTest, test_value.type.Signed_Int == value->type.Signed_Int);
    break;
  case BACNET_APPLICATION_TAG_REAL:
    ct_test(pTest, test_value.type.Real == value->type.Real);
    break;
  case BACNET_APPLICATION_TAG_ENUMERATED:
    ct_test(pTest, test_value.type.Enumerated == value->type.Enumerated);
    break;
  case BACNET_APPLICATION_TAG_DATE:
    ct_test(pTest, test_value.type.Date.year == value->type.Date.year);
    ct_test(pTest, test_value.type.Date.month == value->type.Date.month);
  ...
  case BACNET_APPLICATION_TAG_TIME:
    ...
    ct_test(pTest, test_value.type.Time.hundredths == value->type.Time.hundredths);
    break;
  case BACNET_APPLICATION_TAG_OBJECT_ID:
    ct_test(pTest, test_value.type.Object_Id.type == value->type.Object_Id.type);
    ct_test(pTest, test_value.type.Object_Id.instance == value->type.Object_Id.instance);
    break;
  default:
    break;
  }
