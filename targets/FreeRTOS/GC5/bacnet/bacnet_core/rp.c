...
int rp_decode_apdu(uint8_t *apdu, unsigned apdu_len, uint8_t *invoke_id, BACNET_READ_PROPERTY_DATA *rpdata) {
  ...
  *invoke_id = apdu[2]; /* invoke id - filled in by net layer */
  ...
  if (apdu_len > offset) {
    len = rp_decode_service_request(&apdu[offset], apdu_len - offset, rpdata);
  }
...
int rp_ack_decode_apdu(uint8_t *apdu, int apdu_len, /* total length of the apdu */
  ...
  *invoke_id = apdu[1];
  ...
  offset = 3;
  if (apdu_len > offset) {
    len = rp_ack_decode_service_request(&apdu[offset], apdu_len - offset, rpdata);
  }
...
void testReadPropertyAck(Test *pTest) {
  ...
  rpdata.application_data_len = encode_bacnet_object_id(&apdu2[0], rpdata.object_type, rpdata.object_instance);
