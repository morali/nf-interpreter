...
static int bvlc_encode_read_bdt_ack(uint8_t *pdu, uint16_t max_pdu) {
  ...
  for (i = 0; i < MAX_BBMD_ENTRIES; i++) {
    if (BBMD_Table[i].valid) {
      ...
      len = bvlc_encode_address_entry(&pdu[pdu_len], &BBMD_Table[i].dest_address, BBMD_Table[i].dest_port, &BBMD_Table[i].broadcast_mask);
      pdu_len += len;
    }
  }
 ...
 * @return number of bytes encoded
 */
static int bvlc_encode_forwarded_npdu(uint8_t *pdu, uint16_t max_pdu, struct sockaddr_in *sin, uint8_t *npdu, unsigned npdu_length) {
  int len = 0;
...
static void bvlc_bdt_forward_npdu(struct sockaddr_in *sin, uint8_t *npdu, uint16_t npdu_length, bool original) {
  ...
  for (i = 0; i < MAX_BBMD_ENTRIES; i++) {
    if (BBMD_Table[i].valid) {
       ...
       * a loop.
       */
      if (BVLC_NAT_Handling && (bip_dest.sin_addr.s_addr == BVLC_Global_Address.s_addr) && (bip_dest.sin_port == bip_get_port())) {
        continue;
      }
...
static bool bvlc_bdt_member_mask_is_unicast(struct sockaddr_in *sin) {
  ...
  for (i = 0; i < MAX_BBMD_ENTRIES; i++) {
    if (BBMD_Table[i].valid) {
      ...
      if ((BBMD_Table[i].dest_address.s_addr == sin->sin_addr.s_addr) && (BBMD_Table[i].dest_port == sin->sin_port)) {
        /* unicast mask? */
...
uint16_t bvlc_receive(BACNET_ADDRESS *src, uint8_t *npdu, uint16_t max_npdu, unsigned timeout) {
  ...
  case BVLC_ORIGINAL_UNICAST_NPDU:
    if ((sin.sin_addr.s_addr == bip_get_addr()) && (sin.sin_port == bip_get_port())) {
      ...
      npdu_len = 0;
    } else if (BVLC_NAT_Handling && (sin.sin_addr.s_addr == BVLC_Global_Address.s_addr) && (sin.sin_port == bip_get_port())) {
 ...
 */
BACNET_BVLC_RESULT bvlc_get_last_result(void) { return BVLC_Result_Code; }
 ...
 */
BACNET_BVLC_FUNCTION bvlc_get_function_code(void) { return BVLC_Function_Code; }
