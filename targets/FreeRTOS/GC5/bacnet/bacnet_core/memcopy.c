 ...
 */
bool memcopylen(size_t offset, size_t max, size_t len) { return ((offset + len) <= max); }
 ...
 * the number of bytes copied.
 */
size_t memcopy(void *dest, void *src, size_t offset, /* where in dest to put the data */
...
void test_memcopy(Test *pTest) {
  ...
  len = memcopy(&buffer[0], &big_buffer[0], 1, sizeof(big_buffer), sizeof(buffer));
  ct_test(pTest, len == 0);
}
