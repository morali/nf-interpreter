...
void address_protected_entry_index_set(uint32_t top_protected_entry_index) { Top_Protected_Entry = top_protected_entry_index; }

void address_own_device_id_set(uint32_t own_id) { Own_Device_ID = own_id; }
...
static struct Address_Cache_Entry *address_remove_oldest(void) {
  ...
  pMatch = Address_Cache;
  while (pMatch <= &Address_Cache[MAX_ADDRESS_CACHE - 1]) {
    if ((pMatch->Flags & (BAC_ADDR_IN_USE | BAC_ADDR_BIND_REQ | BAC_ADDR_STATIC)) == ((uint8_t)(BAC_ADDR_IN_USE | BAC_ADDR_BIND_REQ))) {
      if (pMatch->TimeToLive <= ulTime) { /* Shorter lived entry found */
        ulTime = pMatch->TimeToLive;
        pCandidate = pMatch;
      }
...
bool address_mac_from_ascii(BACNET_MAC_ADDRESS *mac, char *arg) {
  ...
  if ((c == 4) || (c == 5)) {
    ...
    c = sscanf(arg, "%2x:%2x:%2x:%2x:%2x:%2x", &a[0], &a[1], &a[2], &a[3], &a[4], &a[5]);
    if (c == 6) {
      mac->adr[0] = a[0];
      mac->adr[1] = a[1];
      mac->adr[2] = a[2];
...
static void address_file_init(const char *pFilename) {
  ...
  if (pFile) {
    while (fgets(line, (int)sizeof(line), pFile) != NULL) {
      /* ignore comments */
      if (line[0] != ';') {
        if (sscanf(line, "%7ld %79s %5u %79s %4u", &device_id, &mac_string[0], &snet, &sadr_string[0], &max_apdu) == 5) {
...
bool address_device_bind_request(uint32_t device_id, uint32_t *device_ttl, unsigned *max_apdu, BACNET_ADDRESS *src) {
  bool found = false; /* return value */
  struct Address_Cache_Entry *pMatch;
...
/* returns true if device is already bound */
/* also returns the address and max apdu if already bound */
bool address_bind_request(uint32_t device_id, unsigned *max_apdu, BACNET_ADDRESS *src) { return address_device_bind_request(device_id, NULL, max_apdu, src); }
...
bool address_device_get_by_index(unsigned index, uint32_t *device_id, uint32_t *device_ttl, unsigned *max_apdu, BACNET_ADDRESS *src) {
  struct Address_Cache_Entry *pMatch;
  bool found = false; /* return value */
...
bool address_get_by_index(unsigned index, uint32_t *device_id, unsigned *max_apdu, BACNET_ADDRESS *src) {
...
int rr_address_list_encode(uint8_t *apdu, BACNET_READ_RANGE_DATA *pRequest) {
  ...
  while (uiIndex <= uiTarget) {
    ...
    iTemp = (int32_t)encode_application_object_id(&apdu[iLen], OBJECT_DEVICE, pMatch->device_id);
...
void address_cache_timer(uint16_t uSeconds) { /* Approximate number of seconds since last call to this function */
  struct Address_Cache_Entry *pMatch;
...
static void set_file_address(const char *pFilename, uint32_t device_id, BACNET_ADDRESS *dest, uint16_t max_apdu) {
  unsigned i;
  FILE *pFile = NULL;
