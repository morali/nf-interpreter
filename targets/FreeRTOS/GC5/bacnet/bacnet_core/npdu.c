 ...
 */
int npdu_encode_pdu(uint8_t *npdu, BACNET_ADDRESS *dest, BACNET_ADDRESS *src, BACNET_NPDU_DATA *npdu_data) {
 ...
 */
int npdu_decode(uint8_t *npdu, BACNET_ADDRESS *dest, BACNET_ADDRESS *src, BACNET_NPDU_DATA *npdu_data) {
  int len = 0;   /* return value - number of octets loaded in this function */
  uint8_t i = 0; /* counter */
  uint16_t src_net = 0;
  uint16_t dest_net = 0;
...
void iam_handler(uint8_t *service_request, uint16_t service_len, BACNET_ADDRESS *src) {
