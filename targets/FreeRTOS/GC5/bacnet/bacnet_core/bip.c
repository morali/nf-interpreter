...
static ip_addr_t BIP_Address;
/* Broadcast Address - stored in network byte order */
static ip_addr_t BIP_Broadcast_Address;

/** Setter for the BACnet/IP socket handle.
 *
 * @param sock_fd [in] Handle for the BACnet/IP socket.
 */
void bip_set_socket(int sock_fd) { BIP_Socket = sock_fd; }

/** Getter for the BACnet/IP socket handle.
 *
 * @return The handle to the BACnet/IP socket.
 */
int bip_socket(void) { return BIP_Socket; }

bool bip_valid(void) { return (BIP_Socket != -1); }

void bip_set_addr(uint32_t net_address) { BIP_Address.addr = net_address; }

/* returns network byte order */
uint32_t bip_get_addr(void) { return BIP_Address.addr; }

void bip_set_broadcast_addr(uint32_t net_address) { BIP_Broadcast_Address.addr = net_address; }

/* returns network byte order */
uint32_t bip_get_broadcast_addr(void) { return BIP_Broadcast_Address.addr; }
...
static int bip_decode_bip_address(BACNET_ADDRESS *bac_addr, ip_addr_t *address, uint16_t *port) {
  int len = 0;
...
int bip_send_pdu(BACNET_ADDRESS *dest, BACNET_NPDU_DATA *npdu_data, uint8_t *pdu, unsigned pdu_len) {
  ...
  if ((dest->net == BACNET_BROADCAST_NETWORK) || (dest->mac_len == 0)) {
    /* broadcast */
    address.addr = BIP_Broadcast_Address.addr;
    port = BIP_Port;
  ...
  bip_dest.sin_addr.s_addr = address.addr;
  ...
  /* Send the packet */
  bytes_sent = sendto(BIP_Socket, (char *)mtu, mtu_len, 0, (struct sockaddr *)&bip_dest, sizeof(struct sockaddr));
...
void bip_get_my_address(BACNET_ADDRESS *my_address) {
  ...
  if (my_address) {
    ...
    memcpy(&my_address->mac[0], &BIP_Address.addr, 4);
...
void bip_get_broadcast_address(BACNET_ADDRESS *dest) {
  ...
  if (dest) {
    dest->mac_len = 6;
    memcpy(&dest->mac[0], &BIP_Broadcast_Address.addr, 4);
...
uint16_t bip_receive(BACNET_ADDRESS *src, uint8_t *pdu, uint16_t max_pdu, unsigned timeout) {
  ...
  if (select(max + 1, &read_fds, NULL, NULL, &select_timeout) > 0)
    received_bytes = recvfrom(BIP_Socket, (char *)&pdu[0], max_pdu, 0, (struct sockaddr *)&sin, &sin_len);
  else
    return 0;
  ...
  if ((function == BVLC_ORIGINAL_UNICAST_NPDU) || (function == BVLC_ORIGINAL_BROADCAST_NPDU)) {
    ...
    if ((sin.sin_addr.s_addr == BIP_Address.addr) && (sin.sin_port == BIP_Port)) {
      pdu_len = 0;
#if 0
