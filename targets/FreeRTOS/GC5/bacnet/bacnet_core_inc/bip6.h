...
int bip6_send_pdu(BACNET_ADDRESS *dest, BACNET_NPDU_DATA *npdu_data, uint8_t *pdu, unsigned pdu_len);
uint16_t bip6_receive(BACNET_ADDRESS *src, uint8_t *pdu, uint16_t max_pdu, unsigned timeout);
...
int bip6_send_mpdu(BACNET_IP6_ADDRESS *addr, uint8_t *mtu, uint16_t mtu_len);
